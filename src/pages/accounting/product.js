...
class Product extends Form {
	...
	async populateUser() { 
		try {
		  ...
		  const { data: user } = await getProduct(userId);
            const product = user[0];
		    console.log(product);
			 if(product.dateBirth)product.dateBirth = new Date();
		     
			product.firstName =product.contactName.first;
			product.lastName =product.contactName.last;
			product.initials =product.contactName.initials;
			// product.IBAN = product.bankInfo.IBAN;
			// product.bank = product.bankInfo.bank;
			// product.branchOfBank = product.bankInfo.branchOfBank;
			// product.healthcareProviderIdentifierOrganisation = product.professionalInfo.healthcareProviderIdentifierOrganisation;
			// product.healthcareProviderIdentifierIndividual = product.professionalInfo.healthcareProviderIdentifierIndividual;
			// product.treatments = product.professionalInfo.treatments;
			// product.licenseNo  = product.professionalInfo.licenseNo;
			// product.licenseValidTill = product.professionalInfo.licenseValidTill;
			// product.organizationAName = product.membership.organizationAName;
			// product.organizationAMemberNo = product.membership.organizationAMemberNo;
			// product.organizationBName =product.membership.organizationBName;
			// product.organizationBMemberNo =product.membership.organizationBMemberNo;
			// product.idPaper  =product.identification.idPaper;
			// product.idPaperValidTill =product.identification.idPaperValidTill;
...
schema = Joi.object({
	...
	doSubmit = async (user) => {
	    ...
	    try{
			console.log(this.state.data);
			await saveProduct(this.state.data,this.state.imageSrc);
			//console.log(this.state.data);
			this.props.history.push("/clinic/products");
		}catch(ex){
			//if(ex.response && ex.response.status === 404){
			if(ex.response){
				const errors = {...this.state.errors};
