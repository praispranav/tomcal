...
class SchedulerfCal extends Form {
    constructor(props) {
		...
		this.state = {
			...
			headerToolbar: {
                ...
                center: 'title',
                right: 'resourceTimelineDay,resourceTimelineTwoDay,resourceTimeGridWeek'   
			},
			buttonText: {
				today:    'Today',
				month:    'Month',
    ...
    async populateCalendarResources() {
		...
		this.setState({ resources:resources.map((resource)=>({
			id: resource.doctorNo,
			title:resource.doctorUser.contactName.first+" "+resource.doctorUser.contactName.last,
  ...
  render() {
    ...
    return (
      ...
      <FullCalendar
       ...
       plugins={[resourceTimelinePlugin,bootstrapPlugin,interactionPlugin,adaptivePlugin,dayGridPlugin,listPlugin, timeGridPlugin]} 
